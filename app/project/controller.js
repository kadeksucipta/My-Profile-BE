        ...
        if(req.file){
            ...
            src.on("end", async () => {
                try {
                    ...
                    let product = new Project({...payload, image_url: filename})
                    await product.save()
            ...
            let product = new Project(payload);
            await product.save();
            return res.json(product);
        ...
        if(req.file){
            ...
            src.on("end", async () => {
                try {
                    ...
                    let product = await Project.findById(id);
                    ...
                    product = await Project.findByIdAndUpdate(id, payload, {
                        new: true,
                        runValidators: true
                    });
                    return res.json(product);
            ...
            let product = await Project.findByIdAndUpdate(id, payload, {
                new: true,
                runValidators: true
            });
            return res.json(product);
...
const destroy = async (req, res, next) => {
    try {
        let product = await Project.findByIdAndDelete(req.params.id);
        let currentImage = `${config.rootPath}/public/images/${product.image_url}`;
