...
const ProjectTiga = require("./model");
        ...
        if(req.file){
            ...
            src.on("end", async () => {
                try {
                    ...
                    let product = new ProjectTiga({...payload, image_url: filename})
                    await product.save()
            ...
            let product = new ProjectTiga(payload);
            await product.save();
            return res.json(product);
        ...
        if(req.file){
            ...
            src.on("end", async () => {
                try {
                    ...
                    let product = await ProjectTiga.findById(id);
                    ...
                    product = await ProjectTiga.findByIdAndUpdate(id, payload, {
                        new: true,
                        runValidators: true
                    });
                    return res.json(product);
            ...
            let product = await ProjectTiga.findByIdAndUpdate(id, payload, {
                new: true,
                runValidators: true
            });
            return res.json(product);
...
const index = async (req, res, next) => {
    try {
        ...
        let count = await ProjectTiga.find().countDocuments();

        let product = await ProjectTiga
        .find(criteria)
        .skip(parseInt(skip))
...
const destroy = async (req, res, next) => {
    try {
        let product = await ProjectTiga.findByIdAndDelete(req.params.id);
        let currentImage = `${config.rootPath}/public/images/${product.image_url}`;
