...
const ProjectDua = require("./model");
        ...
        if(req.file){
            ...
            src.on("end", async () => {
                try {
                    ...
                    let product = new ProjectDua({...payload, image_url: filename})
                    await product.save()
            ...
            let product = new ProjectDua(payload);
            await product.save();
            return res.json(product);
        ...
        if(req.file){
            ...
            src.on("end", async () => {
                try {
                    ...
                    let product = await ProjectDua.findById(id);
                    ...
                    product = await ProjectDua.findByIdAndUpdate(id, payload, {
                        new: true,
                        runValidators: true
                    });
                    return res.json(product);
            ...
            let product = await ProjectDua.findByIdAndUpdate(id, payload, {
                new: true,
                runValidators: true
            });
            return res.json(product);
...
const index = async (req, res, next) => {
    try {
        ...
        let count = await ProjectDua.find().countDocuments();

        let product = await ProjectDua
        .find(criteria)
        .skip(parseInt(skip))
...
const destroy = async (req, res, next) => {
    try {
        let product = await ProjectDua.findByIdAndDelete(req.params.id);
        let currentImage = `${config.rootPath}/public/images/${product.image_url}`;
